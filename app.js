...
function getSupabaseConfigError(config) {
  ...
  if (url.includes('TU-PROYECTO') || key.includes('TU_SUPABASE')) return 'placeholder';
  try {
  ...
  if (key.length < 80 || !key.startsWith('ey')) return 'anon_key_invalid';
...
function supabaseConfigMessage(code = SUPABASE_CONFIG_ERROR) {
  return ({
    ...
    incomplete: 'Supabase incompleto: falta URL o ANON KEY en config.js.',
    ...
    anon_key_invalid: 'La ANON KEY de Supabase no parece válida. Copiá la anon public key, no service_role.',
...
const api = {
  async init() {
    if (HAS_SUPABASE) {
      sb = window.supabase.createClient(CONFIG.SUPABASE_URL.trim(), CONFIG.SUPABASE_ANON_KEY.trim(), {
...
async function authSubmit(event) {
  ...
  try {
    ...
    $('auth-sync-status').innerHTML = `<strong>No pude autenticar.</strong> ${escapeHTML(msg)}<br><span>Checklist: config.js con Project URL + anon public key, SQL ejecutado, Email provider activo y Site URL configurado en Supabase.</span>`;
  } finally {
