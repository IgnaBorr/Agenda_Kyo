...
function renderCalendar() {
  ...
  for (let d = 1; d <= total; d++) {
    ...
    html += `<div class="cal-cell ${date === todayStr() ? 'today' : ''}" data-action="date-click" data-date="${date}">
      ...
      ${events.slice(0,3).map(calEventChip).join('')}
      ${tasks.slice(0,4).map(calTaskChip).join('')}
      ${(events.length + tasks.length) > 5 ? `<span class="cal-chip more" style="color:var(--text3);background:var(--bg4)">+${events.length + tasks.length - 5} más</span>` : ''}
    </div>`;
  }
...
function calEventChip(e) {
  const color = EVENT_TYPES[e.type]?.color || COLORS.teal;
  const label = `${e.start_time ? `${e.start_time} ` : ''}${escapeHTML(e.title)}`;
...
function calTaskChip(t) {
  ...
  return `<span class="cal-chip cal-chip-task" draggable="true" data-cal-kind="task" data-task-id="${t.id}" data-action="edit-task" title="Click para editar · Arrastrar para mover · borrar fecha desde la tarea" style="color:${color};background:${hexToBg(color)}"><i class="fas fa-check-square"></i> ${escapeHTML(t.title)}</span>`;
}
...
function resetTaskForm(defaults = {}) {
  ...
  $('btn-clear-task-date')?.classList.toggle('hidden', !defaults.id && !defaults.due_date);
}
...
async function moveCalendarItem(item, newDate) {
  ...
  if (item.kind === 'task') {
    ...
    await persist('tasks', task, 'tasks');
    toast(`Tarea movida a ${fmtDate(newDate)}`, 'success');
  }
}
