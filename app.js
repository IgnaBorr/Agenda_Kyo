...
function calTaskChip(t) {
  ...
  return `<span class="cal-chip cal-chip-task" draggable="true" data-cal-kind="task" data-task-id="${t.id}" data-action="edit-task" title="Click para editar · Arrastrar para mover · borrar fecha desde la tarea" style="color:${color};background:${hexToBg(color)}"><i class="fas fa-check-square"></i> ${t.start_time ? approxTimeHTML(t.start_time) + ' ' : ''}${escapeHTML(t.title)}</span>`;
}
