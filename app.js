...
function normalizeBoardLink(l) {
  return { id: l.id || uid('bl'), source_id: l.source_id || l.from || null, target_id: l.target_id || l.to || null, label: l.label || '', created_at: l.created_at || new Date().toISOString(), updated_at: l.updated_at || new Date().toISOString() };
}
...
const api = {
  ...
  async loadAll() {
    ...
    const [projects, tasks, events, notes, boardCards, boardLinks] = await Promise.all([
    ...
    for (const res of [projects, tasks, events, notes, boardCards, boardLinks]) if (res.error) throw res.error;
    state = normalizeState({ projects: projects.data, tasks: tasks.data, events: events.data, daily_notes: notes.data, board_cards: boardCards.data, board_links: boardLinks.data });
...
function boardCardHTML(card) {
  ...
  return `<article class="work-card" data-board-card-id="${card.id}" style="left:${card.x}px;top:${card.y}px;width:${card.width || 240}px;">
...
function renderBoardConnections() {
  ...
  svg.innerHTML = '<defs><marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L9,3 z" /></marker></defs>';
  (state.board_links || []).forEach(link => {
    const a = document.querySelector(`[data-board-card-id="${link.source_id}"]`);
    const b = document.querySelector(`[data-board-card-id="${link.target_id}"]`);
    if (!a || !b) return;
...
async function deleteBoardCardFromModal() {
  ...
  if (!id || !confirm('¿Eliminar esta tarjeta y sus conexiones?')) return;
  state.board_links = state.board_links.filter(l => l.source_id !== id && l.target_id !== id);
  await removeRecord('board_cards', id, 'board_cards');
  saveLocalState();
  closeModal('modal-board-card');
  renderBoard();
}
function toggleBoardLinkMode() {
  ...
  boardLinkSourceId = null;
  $('btn-board-link-mode').classList.toggle('active', boardLinkMode);
  $('work-canvas-wrap').classList.toggle('link-mode', boardLinkMode);
  toast(boardLinkMode ? 'Modo conexión activo: tocá origen y destino.' : 'Modo conexión desactivado');
}
async function createBoardLink(sourceId, targetId) {
  ...
  if (exists) return toast('Esa conexión ya existe', 'error');
  const link = normalizeBoardLink({ id: uid('bl'), source_id: sourceId, target_id: targetId });
  state.board_links.push(link);
  await persist('board_links', link, 'board_links');
  boardLinkSourceId = null;
}
async function removeBoardLinksForCard(cardId) {
  ...
  if (!confirm('¿Quitar todas las conexiones de esta tarjeta?')) return;
...
function createCardFromTemplate(template, x = 140, y = 120) {
  const meta = {
  ...
  const card = normalizeBoardCard({ id: uid('bc'), title: meta[0], text: meta[1], category: template, color: meta[2], x, y });
  state.board_cards.push(card);
  persist('board_cards', card, 'board_cards');
}
...
document.addEventListener('click', (event) => {
  ...
  if (startLink) { event.stopPropagation(); boardLinkMode = true; $('btn-board-link-mode')?.classList.add('active'); $('work-canvas-wrap')?.classList.add('link-mode'); boardLinkSourceId = startLink.dataset.boardCardId; return toast('Ahora tocá la tarjeta destino.'); }
  ...
  if (card && boardLinkMode) {
    event.stopPropagation();
    const id = card.dataset.boardCardId;
    if (!boardLinkSourceId) { boardLinkSourceId = id; card.classList.add('link-source'); return; }
    createBoardLink(boardLinkSourceId, id);
    $$('.work-card').forEach(c => c.classList.remove('link-source'));
  }
});

document.addEventListener('pointerdown', (event) => {
  const handle = event.target.closest('[data-board-drag-handle]');
  if (!handle || boardLinkMode) return;
  const cardEl = handle.closest('.work-card');
  if (!cardEl) return;
  const card = state.board_cards.find(c => c.id === cardEl.dataset.boardCardId);
  if (!card) return;
  const rect = $('work-canvas-wrap').getBoundingClientRect();
  boardDragState = { id: card.id, offsetX: event.clientX - rect.left - card.x, offsetY: event.clientY - rect.top - card.y };
...
document.addEventListener('pointermove', (event) => {
  ...
  if (!card) return;
  card.x = clamp(event.clientX - rect.left - boardDragState.offsetX, 16, 4000);
  card.y = clamp(event.clientY - rect.top - boardDragState.offsetY, 16, 2500);
...
document.addEventListener('drop', (event) => {
  ...
  event.preventDefault();
  const rect = wrap.getBoundingClientRect();
  createCardFromTemplate(template, event.clientX - rect.left, event.clientY - rect.top);
});
