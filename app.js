...
function bindEvents() {
  ...
  document.addEventListener('keydown', e => {
    if (e.key === 'Escape') {
      ...
      if (boardLinkMode) toggleBoardLinkMode(false);
...
function toggleBoardLinkMode(force = null) {
  boardLinkMode = force === null ? !boardLinkMode : Boolean(force);
...
async function createBoardLink(sourceId, targetId) {
  ...
  boardLinkSourceId = null;
  boardLinkMode = false;
  $('btn-board-link-mode')?.classList.remove('active');
  $('work-canvas-wrap')?.classList.remove('link-mode');
...
async function handleBoardContextAction(action) {
  ...
  if (action === 'connect' && card) {
    boardLinkMode = true;
    boardLinkSourceId = card.id;
    $('btn-board-link-mode')?.classList.add('active');
    $('work-canvas-wrap')?.classList.add('link-mode');
    toast('Ahora tocá la tarjeta destino.');
  }
...
document.addEventListener('click', (event) => {
  ...
  if (card) selectedBoardCardId = card.dataset.boardCardId;
...
document.addEventListener('pointerdown', (event) => {
  ...
  if (cardPick) selectedBoardCardId = cardPick.dataset.boardCardId;
