...
const LOCAL_KEY = 'umi_agenda_v61_state';
const LEGACY_KEY = 'umi_agenda_v6_state';
...
function eventStartDate(e) { return e?.event_date || null; }
function eventEndDate(e) { return e?.end_date || e?.event_date || null; }
function eventOccursOn(e, dateStr) {
  const start = eventStartDate(e);
  const end = eventEndDate(e);
  return Boolean(start && dateStr && start <= dateStr && dateStr <= end);
}
function eventSpansMultipleDays(e) {
  const start = eventStartDate(e);
  const end = eventEndDate(e);
  return Boolean(start && end && end > start);
}
function eventRangeLabel(e) {
  const start = eventStartDate(e);
  const end = eventEndDate(e);
  if (!start) return 'Sin fecha';
  if (!end || end === start) return fmtDate(start);
...
function isPastEvent(e) { return eventEndDate(e) && eventEndDate(e) < todayStr(); }
function isUpcomingOrActiveEvent(e) { return eventEndDate(e) && eventEndDate(e) >= todayStr(); }
...
function migrateLegacyState(oldState) {
  ...
  if (Array.isArray(oldState.events)) {
    migrated.events = oldState.events.map(e => ({
      ...
      event_date: e.date || e.event_date || todayStr(), end_date: e.end_date || e.endDate || e.date || e.event_date || todayStr(), start_time: e.time || e.start_time || null, end_time: e.end_time || null,
...
function normalizeEvent(e) {
  const start = e.event_date || e.date || todayStr();
  const end = e.end_date || e.endDate || start;
  return {
    id: e.id || uid('e'), project_id: e.project_id || null, title: e.title || 'Evento', notes: e.notes || '', event_date: start,
    end_date: end && start && end < start ? start : end,
...
function getProjectMetrics(projectId) {
  ...
  const upcomingEvents = events.filter(isUpcomingOrActiveEvent);
  ...
  const nextEvent = upcomingEvents.slice().sort((a,b) => `${eventStartDate(a) || '9999'} ${a.start_time || '99:99'}`.localeCompare(`${eventStartDate(b) || '9999'} ${b.start_time || '99:99'}`))[0] || null;
...
function renderToday() {
  ...
  const dayEvents = state.events.filter(e => eventOccursOn(e, today)).sort((a,b) => (a.start_time || '99:99').localeCompare(b.start_time || '99:99'));
...
function eventRowHTML(e) {
  ...
  const range = eventSpansMultipleDays(e) ? ` · ${eventRangeLabel(e)}` : '';
  return `<div class="event-row" data-action="edit-event" data-event-id="${e.id}">
    ...
    <div>
      ...
      <div class="event-card-meta"><i class="fas ${type.icon}" style="color:${color}"></i> ${escapeHTML(type.label)}${range}${e.location ? ` · ${escapeHTML(e.location)}` : ''}</div>
    </div>
...
function renderCalendar() {
  ...
  for (let d = 1; d <= total; d++) {
    ...
    const events = state.events.filter(e => eventOccursOn(e, date)).sort((a,b) => (a.start_time || '').localeCompare(b.start_time || ''));
    ...
    html += `<div class="cal-cell ${date === todayStr() ? 'today' : ''} ${hasMany}" data-action="date-click" data-date="${date}">
      ...
      ${events.map(e => calEventChip(e, date)).join('')}
...
function calEventChip(e, date) {
  const color = eventTypeColor(e.type);
  const start = eventStartDate(e);
  const end = eventEndDate(e);
  const segment = start === end ? 'range-single' : date === start ? 'range-start' : date === end ? 'range-end' : 'range-middle';
  const label = `${date === start && e.start_time ? `${approxTime(e.start_time)} ` : ''}${escapeHTML(e.title)}`;
  const title = eventSpansMultipleDays(e)
    ? `Click para editar · Arrastrar para mover bloque completo · ${eventRangeLabel(e)}`
    : 'Click para editar · Arrastrar para mover';
  return `<span class="cal-chip cal-chip-event ${segment}" draggable="true" data-cal-kind="event" data-event-id="${e.id}" data-action="edit-event" title="${title}" style="color:${color};background:${hexToBg(color)}"><i class="fas fa-calendar-day"></i> ${label}</span>`;
}
...
function renderProjects() {
  ...
  $('projects-grid').innerHTML = projects.map(p => {
    ...
    const nextEventHTML = m.nextEvent ? `<div class="project-calendar-line"><i class="fas fa-calendar-day"></i><span>Próximo: ${eventRangeLabel(m.nextEvent)}${m.nextEvent.start_time ? ' · ' + approxTime(m.nextEvent.start_time) : ''} · ${escapeHTML(m.nextEvent.title)}</span></div>` : '';
...
function resetEventForm(defaults = {}) {
  ...
  $('event-end-date').value = defaults.end_date || defaults.event_date || todayStr();
...
async function saveEventFromForm(event) {
  ...
  const startDate = $('event-date').value || null;
  let endDate = $('event-end-date').value || startDate || null;
  if (startDate && endDate && endDate < startDate) endDate = startDate;
  const item = normalizeEvent({
    ...(existing || {}), id, title: $('event-title').value.trim(), event_date: startDate, end_date: endDate,
...
function handleDocumentClick(event) {
  ...
  if (action === 'date-click') {
    ...
    return openEventModal({ event_date: actionEl.dataset.date, end_date: actionEl.dataset.date });
  }
}
...
async function moveCalendarItem(item, newDate) {
  ...
  if (item.kind === 'event') {
    ...
    const oldStart = event.event_date;
    const spanDays = diffDays(oldStart, eventEndDate(event));
    event.event_date = newDate;
    event.end_date = shiftDate(newDate, spanDays);
    event.updated_at = new Date().toISOString();
    await persist('events', event, 'events');
    toast(`Evento movido a ${eventRangeLabel(event)}`, 'success');
    return;
  }
  if (item.kind === 'task') {
...
function bindEvents() {
  ...
  $('btn-new-event').addEventListener('click', () => openEventModal({ event_date: todayStr(), end_date: todayStr() }));
  $('btn-calendar-event').addEventListener('click', () => openEventModal({ event_date: todayStr(), end_date: todayStr() }));
  ...
  $('event-date')?.addEventListener('change', () => {
    ...
    if (!$('event-end-date').value || $('event-end-date').value < start) $('event-end-date').value = start;
  });
  $('task-due-date')?.addEventListener('change', () => {
    const start = $('task-due-date').value;
    if (!$('task-end-date').value || $('task-end-date').value < start) $('task-end-date').value = start;
  });
